...
object Day05 {
  ...
  def run(): Task[Unit] = {
    for {
      _ <- Console.printLine("--- Day 5: If You Give A Seed A Fertilizer ---")
      _ <-
        Console.printLine(
  ...
  object Seeds {
    ...
    def from(lines: List[String]): Task[Seeds] = {
      lines match {
        case Nil =>
        ...
        case head :: Nil =>
          ZIO.fail(new IllegalStateException(s"Unable to parse seeds: $head"))

        case multiple =>
          ZIO.fail(
            new IllegalStateException(
              s"Unable to parse seeds. Too many lines: $lines"
            )
          )
      }
    }
  ...
  val INPUT_A: String =
    ...
    |2446452559 2629702985 182296534""".stripMargin
}
