...
/** --- Day 1: Trebuchet?! ---
  ...
  *
  * --- Part Two --- Your calculation isn't quite right. It looks like some of
  * the digits are actually spelled out with letters: one, two, three, four,
...
object Day01 {
  ...
  def run(): ZIO[Any, IOException, Unit] = {
    for {
      _ <- Console.printLine("---- Day 1: Trebuchet? ---")
      _ <- Console.printLine(
             "What is the sum of all of the calibration values?"
           )
      a <- partA().timed
  ...
  def partB(): UIO[Int] = ZIO.succeed {
    ...
    val reversedTokens = tokens.map(_.reverse)

    Source
      .fromString(_INPUT_A)
      .getLines()
      .foldLeft(0) { case (acc, line) =>
        val left  = findDigit(line.toList, tokens)
        val right = findDigit(line.reverse.toList, reversedTokens)
        val sum   = (left * 10) + right
  ...
  def findDigit(chars: List[Char], tokens: Array[Vector[Char]]): Int = {

    def validFragStart(frag: Vector[Char]): Vector[Char] = {
      if (frag.isEmpty) {
        frag
      } else {
        if (tokens.exists(_.startsWith(frag))) {
          frag
        } else {
          validFragStart(frag.tail)
        }
      }
    }

    def rec(chars: List[Char], frag: Vector[Char]): Int = {
      chars match {
        case Nil => ???
        ...
        case c :: tail =>
          val frag2 = frag.appended(c)

          // Check for an exact match. 
          val pos = tokens.iterator.indexWhere(_ == frag2)
          if (pos >= 0) {
            pos + 1
          } else {
            val frag3 = validFragStart(frag2)
            rec(tail, frag3)
          }
      }
    }
  ...
  val _INPUT_A =
    """8eight1
      ...
      |eight1eighteight8
      |8four419eighteight1bpv""".stripMargin
}
