...
/** --- Day 1: Sonar Sweep ---
  ...
  *
  * --- Part Two --- Considering every single measurement isn't as useful as you
  * expected: there's just too much noise in the data.
  *
  * Instead, consider sums of a three-measurement sliding window. Again
  * considering the above example:
  *
  * 199 A 200 A B 208 A B C 210 B C D 200 E C D 207 E F D 240 E F G 269 F G H
  * 260 G H 263 H Start by comparing the first and second three-measurement
  * windows. The measurements in the first window are marked A (199, 200, 208);
  * their sum is 199 + 200 + 208 = 607. The second window is marked B (200, 208,
  ...
  * (increased) In this example, there are 5 sums that are larger than the
  * previous sum.
  *
...
object Day01 {
  ...
  def run(): ZIO[Any, IOException, Unit] = {
    for {
      _ <- Console.printLine("--- Day 1: Sonar Sweep ---")
      _ <- Console.printLine(
             "How many measurements are larger than the previous measurement?"
           )
      a <- partA().timed
